	...
	function loadAgain() {
		$.getJSON('http://localhost:8890/trento/api/me', function(data){ //js/sample2.json
			...
			$.each(data, function(i, field){
				...
				$('#url').html('Check it out at '+data[i].user.url);
				$('#join').html('Playing here since '+moment(data[i].user.created_at).toNow());

				// Da Tweet
				$("#listed ul").append('<li id="'+data[i].id+'"><div class="col-md-2"><img src="'+data[i].user.profile_image_url+'" alt="avatar" class="img-rounded img-responsive"></div><div class="col-md-9"><h2>'+data[i].text+'</h2><p>'+moment(data[i].created_at).toNow()+' by <a href="javascript:;" data-toggle="modal" data-target="#myModal" >'+data[i].user.screen_name+'</a></p><p id="options'+data[i].id+'"></p></div></li>').addClass('animated bounceInUp');
				...
				// Like
				$('#options'+data[i].id).append('<a href="/api/loveIt/'+data[i].id+'" data-toggle="tooltip" data-placement="bottom" title="Loves... Likes... Favorites... All verbs means the same"><i class="fa fa-heart"></i></a> '+ data[i].favorite_count+'');
				// Delete
				$('#options'+data[i].id).append('<a href="/api/delete/'+data[i].id+'" data-toggle="tooltip" data-placement="bottom" title="oh boy! delete! delete! delete!"><i class="fa fa-trash"></i></a> ');
				// More
				$('#options'+data[i].id).append('<a href="api/more/'+data[i].id+'" data-toggle="tooltip" data-placement="bottom" title="More option will be great!"><i class="fa fa-ellipsis-h"></i></a> ');
			});
		});
	};
});